    ...
    def _check_expected_price(self):
        for record in self:
            if record.expected_price <= 0:
                raise ValidationError(_("El precio esperado debe ser positivo."))
    ...
    def action_cancel(self):
        for record in self:
            if record.state == "sold":
                raise UserError(_("No se puede cancelar una propiedad vendida."))
            record.state = "cancelled"

    def get_total_expected_price(self, property_type_id):
        properties = self.search([("property_type_id", "=", property_type_id)])
